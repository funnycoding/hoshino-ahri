...
async def cherulize4(session:CommandSession):
    ...
    # 获取鲤鱼王保护协会的参数
    response = requests.post(url='https://service-kjcbcnmw-1254119946.gz.apigw.tencentcs.com/name/0',data=json.dumps({'clanName':'鲤鱼王保护协会'}),headers={'Content-Type':'application/json'})
...
async def update_seeker3():
    ...
    line += '------【当前我会详情👇】------'
    resultString = line +"\n" +  "鲤鱼王保护协会当前排名：" + "【 " +str(rank) +' 】'+ '当前分数' + "【" + str(round(damage)) +"】" + "人均分数【" +str(avgDamage) + "】"
